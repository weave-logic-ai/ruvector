...
impl NeuralPatternStore {
    ...
    pub fn create(config: SonaConfig) -> Result<Self, PatternStoreError> {
        ...
        let store =
            RvfStore::create(&config.store_path(), rvf_options).map_err(PatternStoreError::Rvf)?;
    ...
    pub fn store_pattern(
        ...
        let metadata = vec![
            ...
            MetadataEntry {
                field_id: FIELD_NAME,
                value: MetadataValue::String(name.to_string()),
            },
            MetadataEntry {
                field_id: FIELD_CONFIDENCE,
                value: MetadataValue::F64(confidence),
            },
            MetadataEntry {
            ...
            MetadataEntry {
                field_id: FIELD_TYPE,
                value: MetadataValue::String(TYPE_PATTERN.to_string()),
            },
        ];
    ...
    fn enrich_results(&self, results: &[rvf_runtime::SearchResult]) -> Vec<NeuralPattern> {
        results
            .iter()
            .map(|r| match self.patterns.get(&r.id) {
...
mod tests {
    ...
    fn store_and_search_patterns() {
        ...
        store
            .store_pattern("convergent", "thinking", &[1.0, 0.0, 0.0, 0.0], 0.9)
            .unwrap();
        store
            .store_pattern("divergent", "thinking", &[0.0, 1.0, 0.0, 0.0], 0.7)
            .unwrap();
        store
            .store_pattern("lateral", "creative", &[0.0, 0.0, 1.0, 0.0], 0.8)
            .unwrap();
    ...
    fn get_by_category() {
        ...
        store
            .store_pattern("p1", "alpha", &make_embedding(1.0), 0.9)
            .unwrap();
        store
            .store_pattern("p2", "beta", &make_embedding(2.0), 0.7)
            .unwrap();
        store
            .store_pattern("p3", "alpha", &make_embedding(3.0), 0.8)
            .unwrap();
    ...
    fn update_confidence() {
        ...
        let id = store
            .store_pattern("p1", "cat", &make_embedding(1.0), 0.5)
            .unwrap();
    ...
    fn get_top_patterns() {
        ...
        store
            .store_pattern("low", "cat", &make_embedding(1.0), 0.3)
            .unwrap();
        store
            .store_pattern("high", "cat", &make_embedding(2.0), 0.9)
            .unwrap();
        store
            .store_pattern("mid", "cat", &make_embedding(3.0), 0.6)
            .unwrap();
    ...
    fn get_top_more_than_available() {
        ...
        store
            .store_pattern("only", "cat", &make_embedding(1.0), 0.5)
            .unwrap();
